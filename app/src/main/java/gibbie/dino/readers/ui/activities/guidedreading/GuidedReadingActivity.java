...
public class GuidedReadingActivity extends AppCompatActivity implements RecognitionListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_guided_reading);
        init();
        loadPageData();
        if(isReadToMe){
            initTextToSpeech();
    ...
    private void showBottomPanel(){
        for (int i = 0; i < activeTextViews.size(); i++) {
            Spannable textWithHighlights = new SpannableString(activeTextViews.get(i).getText().toString());
            activeTextViews.get(i).setText(textWithHighlights);
        }
...
//
//                    final ProgressDialog waiting = ProgressDialog.show(GuidedReadingActivity.this,
...
//    }
    ...
    private void startTextToSpeech(){
        ...
        String displaySentence = pages.get(current_page-1).getFullText();
        sentences = displaySentence.split("[.!?]");
        for(int i = 0; i < punctuation.length; i++){
            text = sentences[current_sentence - 1].toLowerCase().replaceAll(punctuation[i],"");
        }
        String displayLine = pages.get(current_page-1).getLines().get(currentTextLine).getText().trim();
        ...
        tts.setOnUtteranceProgressListener(new UtteranceProgressListener() {
            ...
            public void onDone(String utteranceId) {
                ...
                if(onSingleWordSession || onWrongSession){
                    runOnUiThread(() -> startRecording());
                    return;
                }

                if(currentTextLine < activeTextViews.size() - 1){
                    if(!isRecordingSession){
//                        current_sentence++;
                        currentTextLine++;
                        lineReduction = 0;
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            showBottomPanel();
                            String prevText = pages.get(current_page-1).getLines().get(currentTextLine).getText().trim();
                            activeTextViews.get(currentTextLine).setText(prevText);
                        }
                    });
                }
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if(currentTextLine > 0 && currentTextLine < activeTextViews.size() - 1)
                        {
                            Spannable textWithHighlights = new SpannableString(displayLine);
                            activeTextViews.get(currentTextLine-1).setText(textWithHighlights);
                        }
                    }
                });
            ...
            public void onRangeStart(String utteranceId, final int start, final int end, int frame) {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Spannable textWithHighlights = new SpannableString(displayLine);
                        if(currentTextLine > 0 && startIndex > 0)
                            startIndex -= 1;
                        int stopIndex = Math.min(endIndex, displayLine.length());
                        textWithHighlights.setSpan(new ForegroundColorSpan(Color.BLUE), startIndex, stopIndex, Spanned.SPAN_INCLUSIVE_INCLUSIVE);

                        if(currentTextLine > 0){
                            String prevText = pages.get(current_page-1).getLines().get(currentTextLine-1).getText().trim();
                            activeTextViews.get(currentTextLine - 1).setText(prevText);
                        }

                        activeTextViews.get(currentTextLine).setText(textWithHighlights);
                    }
                });
            }
        });
...
//            tts.speak(text, TextToSpeech.QUEUE_FLUSH, null, TextToSpeech.Engine.KEY_PARAM_UTTERANCE_ID);
            if(displayLine.replace("\n", "").replace("\r", "").replace("\"", "").replaceAll("[“”!?.,]", "").trim().length() > 1){
                tts.speak(displayLine, TextToSpeech.QUEUE_FLUSH, null, TextToSpeech.Engine.KEY_PARAM_UTTERANCE_ID);
            }
            else{
                tts.speak("", TextToSpeech.QUEUE_FLUSH, null, TextToSpeech.Engine.KEY_PARAM_UTTERANCE_ID);
            }
        }
    }
...
//        btn_speaker.setVisibility(isEnable? View.VISIBLE : View.GONE);
    ...
    private void pronunciationCheck(){
        ...
        if(!onSingleWordSession){
            ...
            if(isRight){
                page_text.setText( Html.fromHtml("<font color=" + matchColor + ">" + sentences[current_sentence - 1].trim() + "</font>"));
...
//        int point = Math.round(((float) total_right_word_count / total_word_count) * 10);
    ...
    private void goToQuiz(){
        if(quizList.size() > 0) {
            Intent intent = new Intent(GuidedReadingActivity.this, QuizActivity.class);
